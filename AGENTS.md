...
2. **容器启动时**：entrypoint.sh 将 `/opt/preinstall/.config/opencode/` 复制到 `~/.config/opencode/`
3. **OpenCode 配置合并**：如果存在 `preinstall/preinstall-opencode.json`，启动时将其合并到 `~/.config/opencode/opencode.json`
4. **合并规则**：数组字段去重合并，新字段补充，已有非数组字段保留用户原值
